...
func (a *App) RedeemCDKey(cdkey string) error {
	...
	// 1. 基本校验机制（非常简单：比如前缀必须是 BOOST-，并且后面加上一个特定的哈希位能匹配）
	// 生成规则我们在 keygen 里实现。校验规则：
	// 假设 cdkey 长这样: BOOST-XXXX-XXXX-XXXX-XXXX-CHECKSUM
	// 为了最简单的极简方案，我们这就不搞太复杂的非对称，纯用带盐的 SHA256 截断作为校验和。
	if !strings.HasPrefix(cdkey, "BOOST-") {
	...
	payload := strings.Join(parts[:checksumIndex], "-") // "BOOST-XXXX-XXXX..."
