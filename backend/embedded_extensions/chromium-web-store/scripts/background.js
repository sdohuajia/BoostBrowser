...
// Boost Browser 一次性 migration：v1.2.2 的 helper 默认 manually_install=true，
// 已经把 true 写进了用户的 chrome.storage.sync。改默认为 false 不会覆盖已存的值，
// 必须显式 reset 一次，旧 profile 才能切回 newTabUrl 路径（配合 chrome://flags
// extension-mime-request-handling@2，弹原生"添加扩展程序？"对话框）。
// 用 storage.local 的 boolean 标记保证只跑一次，不影响用户后续手动改设置。
function ensureBoostBrowserDefaultsMigration(callback) {
    chrome.storage.local.get({ boost_migration_v1: false }, (m) => {
        if (m.boost_migration_v1) {
            callback && callback();
            return;
        }
        chrome.storage.sync.set({ manually_install: false }, () => {
            chrome.storage.local.set({ boost_migration_v1: true }, () => {
                callback && callback();
            });
        });
    });
...
function startupTasks() {
    ensureBoostBrowserDefaultsMigration(() => {});
...
const msgHandler = function (request, sender, sendResponse) {
    ...
    if (request.boostInstallCrxUrl) {
        // Content script 把 crxUrl 转发过来，由 service worker 在扩展上下文里 fetch
        // （绕过页面 CORS）。完成后 sendResponse({ok:true} | {ok:false, status, error})。
        boostInstallFromStore(request.boostInstallCrxUrl)
            .then((result) => {
                try { sendResponse(result); } catch (e) {}
            })
            .catch((e) => {
                try { sendResponse({ ok: false, status: 0, error: String(e) }); } catch (e2) {}
            });
